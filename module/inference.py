...
class MultiModelRunner:
    def __init__(self,
        ...
        self.samples_per_chunk = int(audio_chunk_seconds * self.audio_sample_rate)
        ...
        # Core models - all using GPU if available
        print("[MultiModelRunner] Loading models...")
        self.yolo = reg.get_face_detector()
        self.face_lm = reg.get_face_landmarker(cpu=self.mediapipe_cpu)
        self.pose_lm = reg.get_pose_landmarker(cpu=self.mediapipe_cpu)
        self.gaze_model, self.gaze_device = reg.get_gaze_model()
        self.whisper = reg.get_whisper_model(whisper_size, device=self.device)
        
        # Sentiment pipeline (0 = GPU:0, -1 = CPU)
        sentiment_device = 0 if self.device == "cuda" else -1
        self.sentiment = reg.get_sentiment_pipeline(device=sentiment_device)
        
        self.return_word_ts = whisper_word_timestamps
        print("[MultiModelRunner] All models loaded!")
        print(f"[MultiModelRunner] Whisper will run every {audio_chunk_seconds}s ({self.samples_per_chunk} samples)")
    ...
    def process(self, sample: Sample) -> Result:
        ...
        if yolo_box is not None:
            ...
            if x2 > x1 and y2 > y1:
                ...
                if crop.size > 0:
                    chw = _crop_norm_224(crop)
                    import torch
                    tensor = torch.from_numpy(chw).unsqueeze(0).to(self.gaze_device)
                    seq = tensor.unsqueeze(0).repeat(1, 7, 1, 1, 1)
                    with torch.inference_mode():
                        gaze_angles, _ = self.gaze_model(seq)
                        yaw, pitch = float(gaze_angles[0, 0]), float(gaze_angles[0, 1])
                    x = -math.cos(pitch) * math.sin(yaw)
                    y = -math.sin(pitch)
                    z = -math.cos(pitch) * math.cos(yaw)
                    gaze_out = {"yaw": yaw, "pitch": pitch, "x": x, "y": y, "z": z}
...
def get_runner(
    ...
    if _global_runner is None:
        ...
        _global_runner = MultiModelRunner(
